using NewParticleSimulator.Render.Objects;
using OpenTK;
using OpenTK.Input;
using OpenTK.Graphics.OpenGL4;

namespace NewParticleSimulator.Render
{
    public class ParticleSimulator
    {
        ...
        public void Run(float dT)
        {
            ...
            if(KeyboardManager.IsKeyDown(Key.ShiftLeft))
            {
                GL.DrawArrays(PrimitiveType.LinesAdjacency, 0, NumParticles);
            }
            if (KeyboardManager.IsKeyDown(Key.ControlLeft))
            {
                GL.DrawArrays(PrimitiveType.TriangleStripAdjacency, 0, NumParticles);
            }
